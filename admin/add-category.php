<?php
/**
 * إضافة فئة جديدة - لوحة التحكم
 * Add new category - Admin panel
 */

$page_title = 'إضافة فئة جديدة';
$page_description = 'إضافة فئة جديدة للمنتجات';

require_once '../config/config.php';
require_once '../classes/Category.php';

// التحقق من صلاحيات المشرف
if (!is_logged_in() || !is_admin()) {
    redirect(SITE_URL . '/login.php');
}

// الحصول على اتصال قاعدة البيانات
$db = getDBConnection();

// إنشاء كائن الفئات
$categoryObj = new Category($db);

$error_message = '';
$success_message = '';

// معالجة النموذج
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $name = sanitize_input($_POST['name']);
    $description = sanitize_input($_POST['description']);
    $image = '';

    // التحقق من صحة البيانات
    if (empty($name)) {
        $error_message = 'اسم الفئة مطلوب';
    } else {
        // معالجة رفع الصورة
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            $upload_dir = '../uploads/categories/';
            
            // إنشاء المجلد إذا لم يكن موجوداً
            if (!file_exists($upload_dir)) {
                mkdir($upload_dir, 0777, true);
            }
            
            $file_extension = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION));
            $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif'];
            
            if (in_array($file_extension, $allowed_extensions)) {
                $new_filename = uniqid() . '.' . $file_extension;
                $upload_path = $upload_dir . $new_filename;
                
                if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) {
                    $image = $new_filename;
                } else {
                    $error_message = 'فشل في رفع الصورة';
                }
            } else {
                $error_message = 'نوع الملف غير مدعوم. يرجى رفع صورة بصيغة JPG, PNG أو GIF';
            }
        }

        // إضافة الفئة إذا لم تكن هناك أخطاء
        if (empty($error_message)) {
            $category_data = [
                'name' => $name,
                'description' => $description,
                'image' => $image
            ];

            if ($categoryObj->addCategory($category_data)) {
                $_SESSION['message'] = ['text' => 'تم إضافة الفئة بنجاح', 'type' => 'success'];
                redirect(SITE_URL . '/admin/categories.php');
            } else {
                $error_message = 'حدث خطأ أثناء إضافة الفئة';
            }
        }
    }
}

// تضمين رأس المشرف
include 'includes/admin_header.php';
?>

<div class="add-category">
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center">
            <h1><i class="fas fa-plus"></i> إضافة فئة جديدة</h1>
            <a href="categories.php" class="btn btn-secondary">
                <i class="fas fa-arrow-right"></i> العودة للفئات
            </a>
        </div>
    </div>

    <div class="row">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5><i class="fas fa-tag"></i> معلومات الفئة</h5>
                </div>
                <div class="card-body">
                    <?php if (!empty($error_message)): ?>
                        <div class="alert alert-danger">
                            <i class="fas fa-exclamation-circle"></i> <?php echo $error_message; ?>
                        </div>
                    <?php endif; ?>

                    <form method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label for="name" class="form-label">اسم الفئة <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="name" name="name" 
                                           value="<?php echo isset($_POST['name']) ? htmlspecialchars($_POST['name']) : ''; ?>" 
                                           required>
                                    <div class="invalid-feedback">
                                        يرجى إدخال اسم الفئة
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label for="description" class="form-label">وصف الفئة</label>
                                    <textarea class="form-control" id="description" name="description" rows="4" 
                                              placeholder="وصف مختصر للفئة..."><?php echo isset($_POST['description']) ? htmlspecialchars($_POST['description']) : ''; ?></textarea>
                                </div>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label for="image" class="form-label">صورة الفئة</label>
                                    <input type="file" class="form-control" id="image" name="image" accept="image/*">
                                    <div class="form-text">
                                        <i class="fas fa-info-circle"></i>
                                        الصيغ المدعومة: JPG, PNG, GIF. الحد الأقصى: 2MB
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-actions">
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-save"></i> حفظ الفئة
                                    </button>
                                    <a href="categories.php" class="btn btn-secondary">
                                        <i class="fas fa-times"></i> إلغاء
                                    </a>
                                </div>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>

        <div class="col-md-4">
            <!-- نصائح -->
            <div class="card">
                <div class="card-header">
                    <h6><i class="fas fa-lightbulb"></i> نصائح مفيدة</h6>
                </div>
                <div class="card-body">
                    <ul class="tips-list">
                        <li>
                            <i class="fas fa-check text-success"></i>
                            اختر اسماً واضحاً ومفهوماً للفئة
                        </li>
                        <li>
                            <i class="fas fa-check text-success"></i>
                            أضف وصفاً مختصراً يوضح محتوى الفئة
                        </li>
                        <li>
                            <i class="fas fa-check text-success"></i>
                            استخدم صورة عالية الجودة تمثل الفئة
                        </li>
                        <li>
                            <i class="fas fa-check text-success"></i>
                            تأكد من أن الصورة بحجم مناسب (مربعة مفضلة)
                        </li>
                    </ul>
                </div>
            </div>

            <!-- معاينة الصورة -->
            <div class="card mt-3">
                <div class="card-header">
                    <h6><i class="fas fa-image"></i> معاينة الصورة</h6>
                </div>
                <div class="card-body text-center">
                    <div id="imagePreview" class="image-preview">
                        <i class="fas fa-image"></i>
                        <p>لم يتم اختيار صورة</p>
                    </div>
                </div>
            </div>

            <!-- إحصائيات -->
            <div class="card mt-3">
                <div class="card-header">
                    <h6><i class="fas fa-chart-bar"></i> إحصائيات الفئات</h6>
                </div>
                <div class="card-body">
                    <?php
                    $categories = $categoryObj->getAllCategories();
                    $total_categories = count($categories);
                    ?>
                    <div class="stat-item">
                        <span class="stat-label">إجمالي الفئات:</span>
                        <span class="stat-value"><?php echo $total_categories; ?></span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<style>
.tips-list {
    list-style: none;
    padding: 0;
}

.tips-list li {
    padding: 0.5rem 0;
    border-bottom: 1px solid #eee;
}

.tips-list li:last-child {
    border-bottom: none;
}

.image-preview {
    width: 100%;
    height: 200px;
    border: 2px dashed #ddd;
    border-radius: 8px;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    color: #999;
    background-color: #f8f9fa;
}

.image-preview img {
    max-width: 100%;
    max-height: 100%;
    border-radius: 4px;
}

.image-preview i {
    font-size: 3rem;
    margin-bottom: 1rem;
}

.stat-item {
    display: flex;
    justify-content: space-between;
    padding: 0.5rem 0;
}

.stat-value {
    font-weight: bold;
    color: #007bff;
}

.form-actions {
    padding-top: 1rem;
    border-top: 1px solid #eee;
}

.form-actions .btn {
    margin-left: 0.5rem;
}
</style>

<script>
// معاينة الصورة
document.getElementById('image').addEventListener('change', function(e) {
    const file = e.target.files[0];
    const preview = document.getElementById('imagePreview');
    
    if (file) {
        const reader = new FileReader();
        reader.onload = function(e) {
            preview.innerHTML = '<img src="' + e.target.result + '" alt="معاينة الصورة">';
        };
        reader.readAsDataURL(file);
    } else {
        preview.innerHTML = '<i class="fas fa-image"></i><p>لم يتم اختيار صورة</p>';
    }
});

// التحقق من صحة النموذج
(function() {
    'use strict';
    window.addEventListener('load', function() {
        var forms = document.getElementsByClassName('needs-validation');
        var validation = Array.prototype.filter.call(forms, function(form) {
            form.addEventListener('submit', function(event) {
                if (form.checkValidity() === false) {
                    event.preventDefault();
                    event.stopPropagation();
                }
                form.classList.add('was-validated');
            }, false);
        });
    }, false);
})();
</script>

<?php include 'includes/admin_footer.php'; ?>
